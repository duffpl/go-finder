...
func TestMulti_ByPath(t *testing.T) {
	...
	t.Run("ReturnFirstResult", func(t *testing.T) {
		mock1.EXPECT().ByPath(testPath).Return("mock-1/mime", nil)
		mock2.EXPECT().ByPath(gomock.Any()).Times(0)
		result, _ := multi.ByPath(testPath)
		assert.Equal(t, "mock-1/mime", result)
	})
	t.Run("CheckUntilMimeHasBeenFound", func(t *testing.T) {
		mock1.EXPECT().ByPath(testPath).Return("", nil)
		mock2.EXPECT().ByPath(testPath).Return("mock-2/mime", nil)
		result, _ := multi.ByPath(testPath)
		assert.Equal(t, "mock-2/mime", result)
	})
	t.Run("ReturnWrappedErrAsSoonAsErrorIsEncountered", func(t *testing.T) {
		mock1.EXPECT().ByPath(testPath).Return("", errors.New("mock-1-error"))
		mock2.EXPECT().ByPath(gomock.Any()).Times(0)
		result, err := multi.ByPath(testPath)
